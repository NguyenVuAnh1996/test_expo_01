...
import getLunarDate from '../../vn_lunar_date/lunarCalendar';

function areEqual(prevProps: DayProps, nextProps: DayProps) {
  const prevPropsWithoutMarkDates = omit(prevProps, 'marking');
  const nextPropsWithoutMarkDates: any = omit(nextProps, 'marking');
...
const Day = React.memo((props: DayProps) => {
  ...
  const lunarDate = _date ? getLunarDate(_date.getDate(), _date.getMonth() + 1, _date.getFullYear()) : null;
  const lunarDateStr = lunarDate ? `${lunarDate.day}/${lunarDate.month}` : '';

  return (
    //@ts-expect-error
    <Component {...props} accessibilityLabel={getAccessibilityLabel} {...dayComponentProps} lunarDate={lunarDateStr}>
